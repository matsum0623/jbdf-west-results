...
export function ClassLink(label:string, date:string, name:string, place:string, class_id:string, disable:boolean, setInfo: (dt:string, name:string, place:string, class_name:string) => void) {
  return disable ? (
    <Link to={`/results/${date}/${class_id}`} className="font-medium text-blue-600 hover:underline"
...
export function Loading(is_loading:boolean) {
  if (is_loading){
    return (
      <div className="loading z-50">
        <div role="status" className="absolute -translate-x-1/2 -translate-y-1/2 top-2/4 left-1/2">
          <svg className="w-8 h-8 text-gray-200 animate-spin fill-blue-600" viewBox="0 0 100 101" fill="none" xmlns="http://www.w3.org/2000/svg">
